...
test("PUT - valid request", async () => {
    ...
    const body: IImportLinksTextRequest = {
        format: DataFormat.ONETAB,
        text: linkGroup.exportOnetab()
    }
    const requestObj = {
        json: async () => (body),
    } as any;
    ...
    expect(json[0].name).toBe('') // Onetab groups aren't named, so `name` is lost
...
test("PUT - ID check", async () => {
    ...
    // Except for `name`, which should be empty now, since Onetab
    // groups aren't named.
    expect(json[0].id).toBe(id)
    expect(json[0].name).toBe('')
    expect(json[0].links.length).toBe(links.length)
